...
class SemanticCache:
    ...
    def get(self, prompt: str, provider: str) -> Optional[Dict[str, Any]]:
        ...
        """
        if not self.redis_client or not self.model:
            return None
    ...
    def set(
        ...
        try:
            ...
            key = f"cache:{provider}:{prompt_hash}"
            
            # Store in Redis with hash
            pipe = self.redis_client.pipeline()
            pipe.hset(key, b"prompt", prompt.encode('utf-8'))
            pipe.hset(key, b"embedding", embedding.astype(np.float32).tobytes())
            pipe.hset(key, b"response", json.dumps(response).encode('utf-8'))
            pipe.expire(key, self.ttl)
            pipe.execute()
            
            logger.info(f"Cached response for {provider} (key: {key})")
            return True
